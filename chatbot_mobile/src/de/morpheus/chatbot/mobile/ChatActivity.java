...
public class ChatActivity extends ActionBarActivity implements OnClickListener, OnInitListener, RecognitionListener {
	...
	private Runnable runnable = new Runnable() {
		   ...
		   public void run() {
			   //Toast t = Toast.makeText(getApplicationContext(),"SilenceTimer aktiviert", Toast.LENGTH_SHORT);
	   		   //t.show();
			   String response = ChatActivity.this.prepareWebServiceCall("<silence/>");
			   ChatActivity.this.communicate(null, response);		
  	   	...
  	   	try {
	   		...
	   		InputStream in = null;
  	   		try{
	  	   		...
	  	   		//URL url = new URL("http://xsmorpheussx.ddns.net:8080/chatbot_service/ws/chat/get");
  	   			URL url = new URL("http://192.168.2.101:8080/chatbot_service/ws/chat/get");
  	   			...
  	   			//t.show();
  	   			//this.communicate("", "Keine Verbindung zum Server");
  	   			e.printStackTrace();
  	   		}
  	   		if(in != null){
	  	   		int character = -1;
